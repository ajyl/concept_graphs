...
class my_dataset(Dataset):
        ...
        if training:
                ...
                if config == "000" and alpha != 1500 and remove_node != "100":
                    ...
                    if dataset == "astronaut-riding-horse":
                        if config == "00":
                            ...
                            prompt = "astronaut"
                        images = np.load(
                            f"../predict_performance/get_images/{prompt}.npy"
                        )
                        new_paths = []
                        for x in images:
                            if not (
                                ...
                                new_paths.append(
                                    f"../predict_performance/get_images/{x[0].split('.')[0]}/{x[1]}.png"
                                )
            ...
            if dataset == "astronaut-riding-horse":
                ...
                self.test_image_paths = glob.glob(
                    f"input/{dataset}/test/{prefix}_{configs}_*{ext}"
                )
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == "single-body_2d_3classes":
            ...
            if self.text:
                label = " ".join(
                    [self.text_map[i][name_labels[i]] for i in range(2)]
                )
            else:
