...
def train(model, iterator, optimizer, criterion, device):
    ...
    for (x, y) in tqdm(iterator, desc="Training", leave=False):
        ...
        y_pred = model(x)
        loss = (
            criterion(y_pred[0], y[0])
            + criterion(y_pred[1], y[1])
...
if __name__ == "__main__":
    ...
    train_dataset = classifier_load_dataset.my_dataset(
        tf,
        5000,
        dataset,
        configs=configs,
        training=True,
        n_class_color=n_class_color,
    )
    test_data = classifier_load_dataset.my_dataset(
        tf,
        500,
        dataset,
        configs=configs,
    ...
    OUTPUT_DIMS = [
        len(properties[key]) for key in ["shapes", "colors", "sizes"]
    ]
    ...
    for epoch in trange(EPOCHS):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(
                model.state_dict(),
                "working/linear-classifier_" + dataset + "_multi-class.pt",
            )
        ...
        print(
            f"\tTrain Loss: {train_loss:.3f} | Train Acc: {train_acc[0]*100:.2f}% {train_acc[1]*100:.2f}% {train_acc[2]*100:.2f}%"
        )
        print(
            f"\tValid Loss: {valid_loss:.3f} | valid Acc: {valid_acc[0]*100:.2f}% {valid_acc[1]*100:.2f}% {valid_acc[2]*100:.2f}%"
        )

    test_loss, test_acc = evaluate(model, test_iterator, criterion, device)
    # print(f'Test Loss: {test_loss:.3f} | Test Acc: {test_acc[0]*100:.2f}%')
    print(
        f"\tTest Loss: {test_loss:.3f} | test Acc: {test_acc[0]*100:.2f}% {test_acc[1]*100:.2f}% {test_acc[2]*100:.2f}%"
    )
